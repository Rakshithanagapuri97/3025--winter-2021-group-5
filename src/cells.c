...
size_t countLiveNeighbours(size_t row, size_t col)
{
	...
	size_t c_row = row % config_NC;
	size_t c_col = col % config_MC;
	for (neighbour_t i = a_posn; i <= h_posn; ++i)
	{
		...
		{
		case a_posn:
			if (c_row > 0 && c_col > 0 && env[row - 1][col - 1] == live)
			{
				cell_count++;
			}
			break;
		...
		case c_posn:
			if (c_row > 0 && c_col < config_ME - 1 && env[row - 1][col + 1] == live)
			{
				cell_count++;
			}
			break;
		...
		case e_posn:
			if (c_col < config_ME - 1 && env[row][col + 1] == live)
			{
				cell_count++;
			}
			break;
		...
		case f_posn:
			if (c_row < config_NE - 1 && c_col > 0 && env[row + 1][col - 1] == live)
			{
				cell_count++;
			}
			break;
		...
		case g_posn:
			if (c_row < config_NC - 1 && env[row + 1][col] == live)
			{
				cell_count++;
			}
			break;
		...
		case h_posn:
			if (c_row < config_NC - 1 && c_col < config_ME - 1 && env[row + 1][col + 1] == live)
			{
				cell_count++;
			}
			break;
	...
	while (1)
	{
		if (reproduction_flag)
		{
			for (size_t i = 0; i != config_NC; ++i)
			{
				for (size_t j = 0; j != config_MC; ++j)
				{
					updateCell(thread_row * config_NC + i, thread_col * config_MC + j);
				}
			}
			usleep(config_TL);
		}
